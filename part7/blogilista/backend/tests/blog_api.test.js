...
beforeAll(async () => {
  ...
  const matti = new User({
    _id: "62966f9f4f30d95f3be4f629",
    username: "Matti",
    name: "Matti",
    passwordHash,
  });
  const teppo = new User({
    _id: "629684298216a36897de9db5",
    username: "Teppo",
    name: "Teppo",
    passwordHash,
  });
  ...
  return (authToken = await helper.testLogin({
    username: "Teppo",
    password: "salasana",
  }));
});
...
describe("when there is initially some blogs saved", () => {
  ...
  describe("changing data of blog", () => {
    ...
    test("returns code 404 if id not valid. no changes to db", async () => {
      ...
      await api.put(`/api/blogs/09623562asd3`).send(addFiveLikes).expect(404);
